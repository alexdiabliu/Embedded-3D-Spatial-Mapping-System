...
//#define PSYSDIV 17
// desired is 26 MHz based on LSD 8

// PLL Configuration Equations:
// f_VCO = (f_XTAL / (Q + 1) / (N + 1)) * MINT, voltage controlled oscillator freq
// SysClk = f_VCO / (PSYSDIV + 1)

// Given:
// f_XTAL = 25 MHz (external crystal frequency on TM4C1294)
// Q = 0 //from lec, predivider
// N = 4 //from lec, second stage divider
// MINT = 96 from data sheet, multiplier

// => f_VCO = (25 MHz / (0 + 1) / (4 + 1)) * 96
//          = (25 / 5) * 96 = 5 * 96 = 480 MHz

// We want:
// SysClk = 26 MHz ˜ f_VCO / (PSYSDIV + 1)
// => PSYSDIV = (480 / 26) - 1 ˜ 17.46 use PSYSDIV = 17
// psydiv is pll system clock divider, decreases PPL speed (f_VC0) to match slower peripherals/cpu

// Final Result:
// SysClk = 480 MHz / (17 + 1) = 26.67 MHz ?
